...
- **Spec baselines for every specification the schemas are built on.**
  ...
  by its text, so an entry can be confirmed against the document rather than
  taken on trust. An entry moves only when the document is re-read and the
  schemas are confirmed against it; git history and this file record when that
  happened.
